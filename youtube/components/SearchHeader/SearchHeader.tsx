import styles from "./SearchHeader.module.css";
import { IoMdNotifications, IoMdSearch, IoMdMic } from "react-icons/io";
import { RiVideoAddFill } from "react-icons/ri";
import { IoApps } from "react-icons/io5";
import { Avatar, IconButton } from "@material-ui/core";
import { MdChevronRight, MdChevronLeft } from "react-icons/md";
import { GrMenu } from "react-icons/gr";
import { useState } from "react";
import router from "next/router";

const SearchHeader = () => {
  const [query, setQuery] = useState("");

  const search = (e) => {
    e.preventDefault();
    router.push(`search?search_query=${query}`);
  };
  return (
    <div className={styles.search__header__top}>
      <IconButton className={styles.search__header__top__nav__button}>
        <GrMenu className={styles.search__header__top__nav__button__icon} />
      </IconButton>
      <img
        onClick={() => router.push("/")}
        alt="logo"
        src="data:image/png;base64,"
      />
      <div className={styles.search__header__top__left}>
        <form className={styles.search__header__top__left__search}>
          <input
            placeholder="Search"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <button type="submit" onClick={search}>
            <IoMdSearch
              className={styles.search__header__top__left__search__icon}
            />
          </button>
        </form>
        <div className={styles.search__header__top__left__search_small}>
          <IoMdSearch
            className={styles.search__header__top__left__search__icon}
          />
        </div>
        <IconButton className={styles.search__header__top__button}>
          <IoMdMic className={styles.search__header__top__icon} />
        </IconButton>
      </div>
      <div className={styles.search__header__top__right}>
        <IconButton className={styles.search__header__top__button}>
          <RiVideoAddFill className={styles.search__header__top__icon} />
        </IconButton>
        <IconButton className={styles.search__header__top__button}>
          <IoApps className={styles.search__header__top__icon} />
        </IconButton>
        <IconButton className={styles.search__header__top__button}>
          <IoMdNotifications className={styles.search__header__top__icon} />
        </IconButton>
        <Avatar className={styles.search__header__avatar} />
      </div>
    </div>
  );
};

export default SearchHeader;
