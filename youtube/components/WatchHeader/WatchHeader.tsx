import styles from "./WatchHeader.module.css";
import { IoMdNotifications, IoMdSearch, IoMdMic } from "react-icons/io";
import { RiVideoAddFill } from "react-icons/ri";
import { IoApps } from "react-icons/io5";
import { Avatar, IconButton } from "@material-ui/core";
import { MdChevronRight, MdChevronLeft } from "react-icons/md";
import { GrMenu } from "react-icons/gr";
import { useState } from "react";
import router from "next/router";
import { IoPersonCircleSharp } from "react-icons/io5";
import { useSession, signIn } from "next-auth/client";

const WatchHeader = () => {
  const [query, setQuery] = useState("");

  const [session, loading] = useSession();

  const search = (e) => {
    e.preventDefault();
    router.push(`search?search_query=${query}`);
  };
  return (
    <div className={styles.watch__header}>
      <div className={styles.watch__header__nav__icon}>
        <IconButton className={styles.watch__header__nav__button}>
          <GrMenu className={styles.watch__header__nav__button__icon} />
        </IconButton>
        <img
          onClick={() => router.push("/")}
          alt="logo"
          src="data:image/png;base64,"
        />
      </div>
      <div className={styles.watch__header__left}>
        <form className={styles.watch__header__left__search}>
          <input
            placeholder="Search"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <button type="submit" onClick={search}>
            <IoMdSearch className={styles.watch__header__left__search__icon} />
          </button>
        </form>
        <div className={styles.watch__header__left__search_small}>
          <IoMdSearch className={styles.watch__header__left__search__icon} />
        </div>
        <IconButton className={styles.watch__header__button}>
          <IoMdMic className={styles.watch__header__icon} />
        </IconButton>
      </div>
      <div className={styles.watch__header__right}>
        <IconButton className={styles.watch__header__button}>
          <RiVideoAddFill className={styles.watch__header__icon} />
        </IconButton>
        <IconButton className={styles.watch__header__button}>
          <IoApps className={styles.watch__header__icon} />
        </IconButton>

        {!session ? (
          <button
            className={styles.watch__header__button}
            onClick={() => signIn()}
          >
            <IoPersonCircleSharp
              className={styles.watch__header__button__icon}
            />{" "}
            SIGN IN
          </button>
        ) : (
          <>
            <IconButton className={styles.watch__header__button}>
              <IoMdNotifications className={styles.watch__header__icon} />
            </IconButton>
            <Avatar
              className={styles.watch__header__avatar}
              alt={session.user.name}
              src={session.user.image}
            />
          </>
        )}
      </div>
    </div>
  );
};

export default WatchHeader;
